...
function renderDashboardEventList(title,list){
  $('dateEvents').innerHTML=`<div class="selected-date-title">${esc(title)} <span class="badge">${list.length} event${list.length===1?'':'s'}</span></div>`+(list.length?list.map(e=>`<div class="date-event-card"><div class="date-event-time"><div>${fmtDate(e.eventDate)}</div><div>${fmtTime(e.eventTime)}</div></div><div class="date-event-info"><b>${esc(e.familyPersonName)} · ${esc(e.eventType)}</b><div class="meta">${esc(e.venueLocation||'-')} · ${esc(e.city||'-')}</div><span class="badge ${esc(e.status)}">${esc(e.status)}</span></div><button type="button" onclick="editEvent('${e.id}')">Open</button></div>`).join(''):'<div class="empty-date">No events found.</div>');
}
...
function renderDateEvents(){
  ...
  $('dateEvents').innerHTML=`<div class="selected-date-title">${esc(title)} <span class="badge">${list.length} event${list.length===1?'':'s'}</span></div>`+(list.length?list.map(e=>`<div class="date-event-card"><div class="date-event-time">${fmtTime(e.eventTime)}</div><div class="date-event-info"><b>${esc(e.familyPersonName)} · ${esc(e.eventType)}</b><div class="meta">${esc(e.venueLocation||'-')} · ${esc(e.city||'-')}</div><span class="badge ${esc(e.status)}">${esc(e.status)}</span></div><button type="button" onclick="editEvent('${e.id}')">Open</button></div>`).join(''):'<div class="empty-date">No events found for this date.</div>');
}
